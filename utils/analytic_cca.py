"""Analytic regularized CCA used only to initialize trainable projectors."""
...
class RegularizedSharedIdentityProjector(nn.Module):
    """Fit paired modalities to one canonical space using regularized CCA."""

    def __init__(self, dim: int = 256, unit_input: bool = False):
        super().__init__()
        self.dim = int(dim)
        self.unit_input = bool(unit_input)
    ...
    def transform(
        self, features: torch.Tensor, modality: str, dimensions: int
    ) -> torch.Tensor:
